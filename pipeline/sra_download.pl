...
# Check if input files were provided, else show usage information
	...
	# process each accession number separately
		...
		if(defined $aspera){
			...
			my $cmd = "";
			if(length($line)==12){
			$cmd = "ascp -QT -l 1000m -P33001 -i ".$myid." era-fasp\@fasp.sra.ebi.ac.uk:/vol1/fastq/".substr($line,0,6).substr($line,9,3)."/".$line."/. .";
			} elsif(length($line)==11){
                       	$cmd = "ascp -QT -l 1000m -P33001 -i ".$myid." era-fasp\@fasp.sra.ebi.ac.uk:/vol1/fastq/".substr($line,0,6)."/0".substr($line,9,2)."/".$line."/. .";
			} elsif(length($line)==10){
			$cmd = "ascp -QT -l 1000m -P33001 -i ".$myid." era-fasp\@fasp.sra.ebi.ac.uk:/vol1/fastq/".substr($line,0,6)."/00".substr($line,9,1)."/".$line."/. .";
			} elsif(length($line)==9){
			$cmd = "ascp -QT -l 1000m -P33001 -i ".$myid." era-fasp\@fasp.sra.ebi.ac.uk:/vol1/fastq/".substr($line,0,6)."/".$line."/. .";
			}
			print($cmd,"\n");
			system($cmd);
			$currentlib = $count-$libcount;
